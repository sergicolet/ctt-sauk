...
export function EjecucionesTable({ ejecuciones }: Props) {
  ...
  function formatHistory(history?: string) {
    if (!history) return <p className="text-muted-foreground italic text-sm">No hay historial disponible.</p>;
                      ...
                      {ej.email_enviado ? (
                        <div className="flex flex-col items-center gap-0.5 text-center">
                          ...
                          <span className="text-[9px] font-bold text-primary uppercase tracking-tight">
                            {(ej.tipo_email || "").toLowerCase().includes("internal") || ej.forzado_interno ? "alertas@sergi.com" : "Draft CTT"}
                          </span>
                        </div>
                      ) : (
            ...
            <div className="space-y-8 pt-6 px-4">
              ...
              <div className="space-y-4 pt-4">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-sm font-extrabold uppercase tracking-wider text-slate-950">Historial Completo</h3>
                </div>
                <div className="border border-slate-200 rounded-xl overflow-hidden shadow-sm">
                  <div className="max-h-[350px] overflow-y-auto p-6 bg-white custom-scrollbar">
                    {formatHistory(selectedEj.historial_formateado)}
                  </div>
                </div>
              </div>
            </div>
